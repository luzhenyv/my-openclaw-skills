...
**每个计划必须有明确的交易逻辑类型（`logic_type`）和投资论点（`thesis`），禁止录入缺乏逻辑的计划。**

创建流程：收集基本信息 → **讨论交易逻辑** → 助手给出逻辑评估/建议 → 用户确认 → 预览 → 写入。

- 引导用户选择 `logic_type`（回调买入、突破追涨、止盈、止损、左右侧建仓等）
- 要求用户阐述具体理由（为什么这个价格？核心假设是什么？）
- 助手主动评估逻辑是否合理、区间是否自洽，提出改进建议
- 拒绝录入纯情绪化表达（如"感觉要涨"、"别人推荐"）

详细流程和示例见 `references/plan_crud_spec.md` 的创建计划章节。
